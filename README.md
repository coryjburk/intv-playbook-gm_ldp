...
* **File:** `Eccles\\\_GM\\\_LDP\\\_Interview\\\_Playbook.html`
...
```
Eccles\\\_GM\\\_LDP\\\_Interview\\\_Playbook.html   ← the entire tool (HTML + CSS + JS in one file)
GM\\\_LDP\\\_Playbook\\\_User\\\_Manual.docx        ← student-facing Word user manual
README.md                                ← this file
```
...
### 4.1 Questions — `GM\\\_QUESTIONS`

Each entry in the `GM\\\_QUESTIONS` array follows this shape:
...
### 4.5 Practice history — `localStorage\\\["gmldp\\\_practice\\\_history\\\_v1"]`
...
Each is entered on a 1-5 scale via a dropdown (`RUBRIC\\\_DIMENSIONS` in the `<script>`
...
1. Open the HTML file in a text editor and locate the `GM\\\_QUESTIONS` array inside
the `<script>` tag.
...
from whatever values exist in `GM\\\_QUESTIONS` — you do not need to edit the filter
